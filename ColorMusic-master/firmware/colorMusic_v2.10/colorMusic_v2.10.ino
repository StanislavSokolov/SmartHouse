...
// ----- настройки ленты
#define NUM_LEDS 225        // количество светодиодов (данная версия поддерживает до 410 штук)
...
#define MLED_ON HIGH
#define LED_PIN_0 2             // пин D0 светодиодной ленты
#define LED_PIN_1 3             // пин D1 светодиодной ленты
#define LED_PIN_2 4             // пин D2 светодиодной ленты
#define LED_PIN_3 5             // пин D3 светодиодной ленты

#define RELAY1 7                // пин управления реле 1
#define RELAY2 9                // пин управления реле 2
#define RELAY3 11               // пин управления реле 3
#define RELAY4 13               // пин управления реле 4

#define MCU_EN 12               // пин разрешение от MCU
#define MCU_CLK 10              // пин синхросигнал от MCU
#define MCU_DATA 8              // пин данные от MCU
//#define MCU_RESERV 6          // пин резерв
...
byte EMPTY_BRIGHT = 50;           // яркость "не горящих" светодиодов (0 - 255)
...
void setup() {
  Serial.begin(9600);
  FastLED.addLeds<WS2811, LED_PIN_0, GRB>(leds, NUM_LEDS).setCorrection( TypicalLEDStrip );
  FastLED.addLeds<WS2811, LED_PIN_1, GRB>(leds, NUM_LEDS).setCorrection( TypicalLEDStrip );
  FastLED.addLeds<WS2811, LED_PIN_2, GRB>(leds, NUM_LEDS).setCorrection( TypicalLEDStrip );
  FastLED.addLeds<WS2811, LED_PIN_3, GRB>(leds, NUM_LEDS).setCorrection( TypicalLEDStrip );
...
void loop() {
  mcuProtocol();     // функция работы с esp32 (последовательный интерфейс)
  mcuModeRelay();    // функция обработки сообщения (определяем режим работы реле) 
  mcuModeLight();    // функция обработки сообщения (определяем режим работы подстветки)
  mainLoop();        // главный цикл обработки и отрисовки 
//  eepromTick();      // проверка не пора ли сохранить настройки
} 

void mcuProtocol() {
  if (digitalRead(MCU_EN) == 1) {
    if (digitalRead(MCU_CLK) == 1 and prev_MCU_CLK == 0) {
      if (digitalRead(MCU_DATA) == 1) {
        bitSet(message, countBit);
      } else {
        bitClear(message, countBit);
      }
        countBit++;
    }
    readyMessage = 0;
  ...
  prev_MCU_CLK = digitalRead(MCU_CLK);
...
void mcuModeLight(){
  if (readyMessage == 1) {
    if (message == 0b00000000) {                  // Основное освещение
      this_mode = 6;
      freq_strobe_mode = 0;
      light_mode = 0;
...
//          break;
//      }
//    } else if (message == 0b00100010) {           // Вправо
//      switch (this_mode) {
//        case 0:
//        case 1: SMOOTH = smartIncrFloat(SMOOTH, 0.05, 0.05, 1);
//          break;
...
//          break;
//      }  
//    } else if (message == 0b00100011) {           // Вверх
//      switch (this_mode) {
//        case 0:
//          break;
...
//          break;
//      }  
//    } else if (message == 0b00100100) {           // Вниз
//      switch (this_mode) {
//        case 0:
//          break;
//        case 1: RAINBOW_STEP = smartIncrFloat(RAINBOW_STEP, -0.5, 0.5, 20);
...
//    } else if (message == 0b00100101) {           // Ок (сохранение настроек)
//      updateEEPROM();   
//    } else if (message == 0b01000000) {           // Стереосистема и основное освещение
      
    } else if (message == 0b10000000) {           // Отключить все
      this_mode = 6;
      freq_strobe_mode = 0;
      light_mode = 0;  
    } else if (message == 0b10100000) {           // Динамическое освещение
      
    } else if (message == 0b10100001) {           // Случайный режим интерьерного освещение
      switch (this_mode) {
        case 4:
    ...
    } else if (message == 0b10100010) {           // Постоянный свет
      this_mode = 6;
      light_mode = 0;
    } else if (message == 0b10100011) {           // Плавная смена цвета
      this_mode = 6;
      light_mode = 1;  
    } else if (message == 0b10100100) {           // Бегущая радуга
      this_mode = 6;
      light_mode = 2;  
    } else if (message == 0b11000000) {           // Стереосистема
      
    } else if (message == 0b11100000) {           // Случайный режим динамического освещения
      switch (this_mode) {
        case 4:
        case 7: if (++freq_strobe_mode > 3) freq_strobe_mode = 0;
          break;
    ...
    } else if (message == 0b11100001) {           // Шкала громкости (градиент)
      this_mode = 0;  
    } else if (message == 0b11100010) {           // Шкала громкости (радуга)
      this_mode = 1;   
    } else if (message == 0b11100011) {           // Цветомузыка (5 полос)
      this_mode = 2;   
    } else if (message == 0b11100100) {           // Цветомузыка (3 полосы)
      this_mode = 3;   
    } else if (message == 0b11100101) {           // Цветомузыка (1 полоса - 3 частоты)
      this_mode = 4;
      freq_strobe_mode = 0;  
    } else if (message == 0b11100110) {           // Цветомузыка (1 полоса - низкие)
      this_mode = 4;
      freq_strobe_mode = 1;   
    } else if (message == 0b11100111) {           // Цветомузыка (1 полоса - средние)
      this_mode = 4;
      freq_strobe_mode = 2;   
    } else if (message == 0b11101000) {           // Цветомузыка (1 полоса - высокие)
      this_mode = 4;
      freq_strobe_mode = 3;   
    } else if (message == 0b11101001) {           // Стробоскоп
      this_mode = 5;   
    } else if (message == 0b11101010) {           // Бегущие частоты (3 частоты)
      this_mode = 7;
      freq_strobe_mode = 0;  
    } else if (message == 0b11101011) {           // Бегущие частоты (низкие)
      this_mode = 7;
      freq_strobe_mode = 1;  
    } else if (message == 0b11101100) {           // Бегущие частоты (средние)
      this_mode = 7;
      freq_strobe_mode = 2;  
    } else if (message == 0b11101101) {           // Бегущие частоты (высокие)
      this_mode = 7;
      freq_strobe_mode = 3;  
    } else if (message == 0b11101110) {           // Анализатор спектра
      this_mode = 8; 
    } else {                                      // Другое
//      this_mode = 6;
//      freq_strobe_mode = 0;
